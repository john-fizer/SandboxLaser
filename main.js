...
class AuthManager {
    ...
    init() {
        // Initialize user DB if empty OR corrupt (quick sanity check)
        const savedUsers = localStorage.getItem('jobflow-users');
        if (!savedUsers) {
            this.resetUsers();
        } else {
            try {
                const users = JSON.parse(savedUsers);
                // Sanity check: Does admin exist and have a role?
                const admin = users.find(u => u.username === 'admin');
                if (!admin || !admin.role) {
                    console.log('Detected stale user data. Resetting user database...');
                    this.resetUsers();
                }
            } catch (e) {
                console.error('User DB corrupt. Resetting.');
                this.resetUsers();
            }
        }
        ...
        if (savedUser) {
            try {
                ...
                // Re-validate against current DB to ensure role allows access
                const dbUser = this.validateCredentials(this.currentUser.username, this.currentUser.password);
                if (dbUser) {
                    this.currentUser = dbUser; // Update session with fresh DB data
...
class JobFlowDashboard {
    ...
    // Persist Jobs
    saveJobs() {
        localStorage.setItem('jobflow-jobs', JSON.stringify(this.jobs));
    }

    loadJobs() {
        const saved = localStorage.getItem('jobflow-jobs');
        if (saved) {
            try {
                this.jobs = JSON.parse(saved);
                // Simple version check or migration could go here if needed
            } catch (e) {
                console.error('Job Data Corrupt, reloading defaults');
                this.loadMockData();
            }
        } else {
            this.loadMockData();
